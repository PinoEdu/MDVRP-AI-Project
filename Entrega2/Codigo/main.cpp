...
vector<Ruta> tabuSearch(vector<Ruta> rutas, vector<Cliente>& clientes, const vector<Deposito>& depositos, mt19937& gen, int tabuTenure, int maxIteraciones) {
    ...
    const int maxIteracionesSinMejora = 50; // Criterio de parada por estancamiento
    ...
    while (iteracion < maxIteraciones && iteracionSinMejora < maxIteracionesSinMejora) {
        ...
        if (swapRealizado) {
            ...
            // Criterio de aspiración: Aceptar el movimiento incluso si es tabú si mejora la mejor solucion
...
int main() {
    ...
    //int numSwaps = elements.clientes.size();
    //int numSwaps = 50;
    ...
    //cout << "Tiempo de ejecución (Tabu Search): " << tiempo_transcurrido << " segundos" << FILE_NAME <<endl;
    ...
    //printClients(elements.clientes);
